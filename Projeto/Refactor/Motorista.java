...
public class Motorista extends Ator implements Serializable
{
     ...
     *Grau de cumprimento de horário estabelecido com o cliente, dado por um fator entre 0 e 100.
     */
    private int grau;
    /** Classificação do motorista, calculada com base na classificacao pelo cliente no final viagem. */
    private int classificacao;
    /** Número de classificações feitas ao cliente. */
    private int numClassi;
    /** Número de kms ja realizados na UMeR. */
    private double kmsTotais;
    /** True -> o motorista está em horario de trabalho; false caso contrário. */
    private boolean disponivel;
    /** Veiculo utilizado pelo motorista. */
    ...
    public void atualizaClassificacao(int classi) throws ValueOutOfBoundsException{
        if(classi < 0 || classi > 100) {
			throw new ValueOutOfBoundsException();
		} else //Primeira classificação ao motorista
		if(this.numClassi == 0){
    ...
    public double tempoViagem(double distancia){
        double v = this.taxi.getVelocidadeMedia();
        return distancia/v;
    }
    ...
    public void atualizaDados(Coordenada novaLoc, double distPercorrida, double fiabilidade, double dif){
        ...
        this.grau = (int)((this.grau * (getHistorico().size()-1) + 100*dif)/ getHistorico().size());
    ...
    public boolean equals(Object o){
        ...
        return super.equals(m) && hashCode() == m.hashCode() && this.grau == m.getGrau() 
    ...
    public int hashCode(){
        ...
        return 37*hash + this.taxi.hashCode();
    }
    ...
    public String toString(){
        ...
        s.append(super.toString()).append("\n" + "Grau de cumprimento de horário: ").append(this.grau)
				.append("\n" + "Classificação do motorista: ").append(this.classificacao).append("\n" + "Número de Kms realizados na UMeR até ao momento: ").append(String.format("%.2f",this.kmsTotais)).append("\n")
         .append("O motorista encontra-se em horário de trabalho: ").append(this.disponivel).append("\n");
        if(taxi != null) {
			s.append("Veiculo:").append(this.taxi);
		}
    ...
    public int compareTo(Motorista m){
        ...
        if(r == 0){
            ...
            if(this.classificacao > c) {
				r = 1;
			} else if(this.classificacao < c) {
				r = -1;
			} else {
				r = 0;
			}
        ...
        if(r == 0) {
			r = this.disponivel == m.getDisponibilidade() ? 0 : (m.getDisponibilidade() ? 1 : -1);
		}
        
        if(r == 0) {
			r = this.taxi.compareTo(m.getVeiculo());
		}
